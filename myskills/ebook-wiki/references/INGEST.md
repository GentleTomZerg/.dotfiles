...
5. **Write the source page** `wiki/sources/<chapter>.md` (only after confirmation): spine blocks in order, then each declared core's slots, then `## Quote check`. Block names and table headers follow the book's language — see [LANGUAGE](LANGUAGE.md).
...
7. **Close the loop.** Update `index.md`, append `log.md` with `## [YYYY-MM-DD] ingest | <chapter> | <chapter_id>`. Then run the structural half of the lint: every `### §n` present, every quote in `## Quote check`, every inbound anchor resolving.

## Notes

- If the chapter's core slots come out empty, do not pad them. An empty slot is either omitted (per [SPINE](SPINE.md) "Empty slots") or evidence that the book's conventions block is wrong — raise the latter with the human and fix the block.
