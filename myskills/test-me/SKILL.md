---
name: test-me
description: Prove you know it — cold recall, graded against the source, with passes promoting your own words onto the page.
...
Locate the vault and read its schema. Tests live in `## Tests` sections on the pages they test; the schema defines their shape and `must include` is already written on each one.

## Steps

1. **Collect what is due.** A test is due when it has no verdict, when its page is `shaky` or `unlearned`, or when its last verdict is older than the review interval (**default 14 days** — the human can change it). The human may name a page or topic to narrow the session; with no argument, take everything due.

   **Completion:** a listed set, with the page and form for each.

2. **Ask cold, one at a time.** State the prompt verbatim. No page open, no source, no paraphrase of the idea in the question, no hint — and **do not show `must include` until after they have answered.** It is a grading key, and showing it turns the test into a reading exercise. Wait for their words.

   **Completion:** they have given an answer, and you have not yet graded it.

3. **Grade against the source, not against the page.** The page is your own compression; grading against it lets the human pass by parroting the wiki back. Check the source when the answer turns on a detail. Then:
...
4. **Write the result.**
   - The verdict and date on the test; the page's `confidence` rolled up from its tests.
   - **On solid** — promote the human's sentence into `## In my words` on the page, marked as theirs. This is the only route by which their words enter the wiki, and it is what makes the page their knowledge rather than a book report.
   - **On partial or not-yet** — add the missing element to `## Notes` with a locator. Their sentence is *not* promoted; it is the version being dislodged.
   - A gap the answer exposed that no page records becomes an `## Open questions` bullet.

   **Completion:** every test asked this session carries a new verdict, and every solid pass produced a promoted sentence.

5. **Count the failures.** A test failed twice is not evidence that the human needs quizzing harder — it is evidence that **pass 1 did not take**. Mark the page `unlearned`, stop testing it, and hand back the real fix: re-read the unit, then `/skill:ingest` it again. On the third pass escalate to a test that connects the idea to a neighbouring one, which is where a half-learned idea usually shows itself.

6. **Log it** — `## [<date>] test | <page> — <n> solid, <n> partial`.

## Test forms

Pick the form that can actually fail for the thing the page holds.
